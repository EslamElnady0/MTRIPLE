import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:mtriple/core/colors/colors.dart';

class Styles {
  static TextStyle style20whiteBold = TextStyle(
      color: Colors.white, fontSize: 20.sp, fontWeight: FontWeight.w700);

  static TextStyle style20white = TextStyle(
      color: Colors.white, fontSize: 20.sp, fontWeight: FontWeight.w500);

  static TextStyle style14whiteSemiBold = TextStyle(
      color: Colors.white, fontSize: 14.sp, fontWeight: FontWeight.w500);

  static TextStyle style14white = TextStyle(
      color: Colors.white, fontSize: 14.sp, fontWeight: FontWeight.w400);

  static TextStyle style12white = TextStyle(
      color: Colors.white, fontSize: 12.sp, fontWeight: FontWeight.w500);

  static TextStyle style16white = TextStyle(
      color: Colors.white, fontSize: 16.sp, fontWeight: FontWeight.w400);

  static TextStyle style24whiteBold = TextStyle(
      color: Colors.white, fontSize: 24.sp, fontWeight: FontWeight.w600);

  static TextStyle style36greenBold = TextStyle(
      color: Colorrs.green, fontSize: 36.sp, fontWeight: FontWeight.w600);
}
